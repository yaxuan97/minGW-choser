...
> **⚠️ 本项目仍在积极开发中。** 核心功能已可用，但 API、参数和输出格式在 v1.0 之前可能发生变化。欢迎反馈和贡献！
...
3. **进行修改** — 遵循现有代码风格并添加测试
4. **运行测试** — `go test ./...` 必须全部通过
5. **提交** — 使用 [约定式提交](https://www.conventionalcommits.org/zh-hans/) 格式
6. **发起 Pull Request** — 清晰描述你改了什么以及为什么

### 可以做些什么

- **新增数据源** — 将 MSYS2、LLVM-MinGW 或其他 MinGW 发行版添加到 `builds.json` 和 `fetch/`
- **新平台支持** — 改进特殊环境的检测（Cygwin、MSYS2、WSL）
- **优化推荐** — 根据实际使用经验调整 `builds.json` 中的评分规则
- **Shell 补全** — 编写 bash/zsh/fish 补全脚本
- **一键下载** — 添加 `--download` 参数直接下载解压推荐构建
- **GUI 封装** — 构建桌面端界面，消费 `--json` 输出
